...
(function () {
    ...
    const paginationWrap     = document.getElementById("categoryPagination");
    ...
    function openModal(id, name) {
        ...
        if (modalText) {
            modalText.textContent = name
                ? 'Delete "' + name + '"? The category and its image will be removed.'
    ...
    if (modalConfirm) {
        modalConfirm.addEventListener("click", function () {
            ...
            fetch(BASE_URL + "ajax/delete-category.php", {
                method: "POST",
                body: formData,
    ...
    function renderRow(c) {
        ...
        return `
            <tr data-id="${c.id}">
                <td>${img}</td>
                <td>
                    <div class="cat-name">${escapeHtml(c.category_name)}</div>
                </td>
                <td>
    ...
    if (paginationControls) {
        paginationControls.addEventListener("click", function (e) {
            ...
            const card = document.querySelector(".category-list-card");
            if (card) {
                window.scrollTo({
    ...
    function loadCategories() {
        tbody.innerHTML = `
            <tr class="loading-row">
                <td colspan="4"><span class="table-spinner"></span></td>
            </tr>
        `;

        if (paginationWrap) paginationWrap.style.display = "none";

        fetch(BASE_URL + "ajax/category-list.php", { credentials: "same-origin" })
            .then(r => r.json().catch(() => ({
                success: false,
                message: "Unexpected server response."
            })))
